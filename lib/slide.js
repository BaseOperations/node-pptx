...
class Slide {
    ...
    async backgroundImage(imageUrl, config) {
        try {
            let image = new Image();
            image.x(0);
            image.y(0);
            ...
            await this.powerPointFactory.setBackgroundImage(this, image);
        } catch (err) {
            throw new Error(`Exception in Slide.backgroundImage() when calling this.processConfig(). ${err.message}`);
        }
    }
