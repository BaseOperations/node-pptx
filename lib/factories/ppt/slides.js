...
class SlideFactory {
    ...
    setBackgroundImage(slide, image, mediaName, rId) {
        let slideKey = `ppt/slides/${slide.name}.xml`;
        let slideContent = this.content[slideKey]['p:sld']['p:cSld'][0];
        let existingNodes = PptxContentHelper.extractNodes(slideContent);

        slideContent['p:bg'] = [{}];
        slideContent['p:bg'][0]['p:bgPr'] = [{}];
        slideContent['p:bg'][0]['p:bgPr'][0]['a:blipFill'] = [{
            'a:blip': {
                $: {
                    'r:embed': rId,
                    'cstate': 'print',
        ...
        slideContent['p:bg'][0]['p:bgPr'][0]['a:stretch'] = [{
            'a:fillRect': {},
        }];
        slideContent['p:bg'][0]['p:bgPr'][0]['a:effectLst'] = [{}];

        PptxContentHelper.restoreNodes(slideContent, existingNodes);
        this.content[`ppt/media/${mediaName}`] = image.source;
    
        // Ensure the mediaName and rId are unique
        const relationshipExists = this.content[`ppt/slides/_rels/${slide.name}.xml.rels`].Relationships.Relationship
        .some(rel => rel.$.Id === rId || rel.$.Target === `../media/${mediaName}`);
