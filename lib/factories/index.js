...
const request = require('postman-request').defaults({ encoding: null });
...
class PowerPointFactory {
    ...
    async setBackgroundImage(slide, image) {
        let imageDownloaded = await new Promise(function(resolve, reject) {
            request.get(image.downloadUrl, { timeout: 30000 }, function(err, _, buffer) {
                if (err) reject(err);
                resolve(buffer);
            });
        });
        image.source = await sharp(imageDownloaded)
                .resize(image._cx)
                .toFormat('png')
                .toBuffer();
        await this.pptFactory.setBackgroundImage(slide, image);
    }
